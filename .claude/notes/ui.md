...
- **L'accent se dérive, il ne se décline pas à la main.** Une seule valeur est stockée (`prefs.accent`) ; `window.applyAccent` en tire `--accent-2` et `--accent-soft` en `color-mix`, et la pose avant le premier rendu, comme le style.
