...
| **Paramètres** | — | `SettingsView` | Compte · Thème (thème **et** couleur d'accent, au même nuancier que les trackers) · Onglets (`TabsSettingsCard` : afficher/masquer **et** réordonner, Log compris) · Affichage (bulles infos, numéro de semaine) · **Archives** (trackers archivés, seul point d'accès pour les désarchiver) · Un retour ? (`FeedbackCard`). Chaque réglage porte sa bulle « i » ; les lignes sont en `.field.spread` — intitulé à gauche, contrôle collé au bord droit, pour que la colonne des bascules s'aligne au lieu d'onduler avec la longueur des noms. |
| *(hors onglets)* | — | `SignIn` | Connexion / création de compte / lien magique / mot de passe oublié. |
| *(hors onglets)* | — | `AtelierView` | L'**atelier**, à l'adresse `#atelier` (app.atelier.jsx) : tout le vocabulaire visuel, rangé par famille et rendu sous n'importe quel thème, hors de la barre d'onglets et hors de l'authentification. `#sink` y mène encore. |
