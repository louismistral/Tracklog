...
| `app.ui.jsx` | **Les briques d'interface partagées** — celles que `ui.md` décrit (`Segmented`, `BoolPill`, `IconBar`, `GearIcon`, `InfoBubble`, `SwatchGrid`, `ColorEditor`) plus celles que `#sink` montre au même titre (`NumPill`, `ChevronDown`, `DragHandle`, `useDragReorder`). ~580 lignes. |
...
| **Style** (`STYLES`, `data-theme`) | Un jeu de variables CSS sous `:root[data-theme="<id>"]`. Il choisit le fond et l'encre, et peut aussi déplacer la **police** (`--font-sans/serif/mono`, `--font-serif-style`, `--font-size-adjust`), les **coins** (`--radius`, `--radius-pill`, `--radius-round`), la **lueur** (`--shadow-card`, `--text-glow`) et l'**interlettrage** (`--tracking-body`). Le `:root{}` de base porte les valeurs d'« Aristide » ; un style ne redéclare que ce qui diffère chez lui — Sombre et Clair ne changent que la couleur. |
...
| **Atelier** (`#sink`) | La page qui rend toutes les briques côte à côte dans tous leurs états. Une **adresse**, pas un onglet : ni compte ni données. Sert à voir ce qu'un changement de style déplace, et à regarder ce qui existe avant d'inventer un contrôle de plus. |
