...
| **Style** (`STYLES`, `data-theme`) | Un jeu de variables CSS sous `:root[data-theme="<id>"]` — il choisit le fond et l'encre. |
...
| **Nuancier** (`SwatchGrid`, `COLORS`) | Dix ronds sur une ligne : sept teintes nommables d'un mot, un gris, l'**encre** (`var(--ink)`), et le « + » qui ouvre l'**éditeur** (`ColorEditor` : teinte/saturation/luminosité + pipette système). |
...
- **Aucun framework CSS** — tout le style est dans le `<style>` de `Tracklog.html`, thème « Aristide » (variables `--bg`, `--ink`, `--accent`…, clair/sombre via `data-theme`).
