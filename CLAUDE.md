...
- **`BoolPill` (app.jsx) est le composant oui/non standard.** Une pill à deux options avec un fond qui glisse (`.bool-pill`/`.bool-pill-thumb`), même famille visuelle que le sélecteur Affiché/Masqué (`.vue-mode`) — utilisé partout où une case à cocher servait un réglage oui/non (Case joker, Période d'activité, Graphe cumulatif, « garder dans mes aliments »). Ne pas réintroduire de `<input type="checkbox">` pour un réglage binaire : passer par `BoolPill`, accessible depuis `app.food.jsx` via le scope global partagé.
