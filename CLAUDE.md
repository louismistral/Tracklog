...
| `app.atelier.jsx` | **L'atelier**, page `#atelier`. Il **relit les cinq `.jsx` et `styles.css` à l'exécution** (`fetch`) et en tire **deux inventaires qui ne se recouvrent pas** : les **composants** (une brique du code, trouvée par son annotation `@atelier`) et les **apparitions** (une combinaison de classes qui existe vraiment quelque part — `button.icon-btn.cal-nav`, qui n'est aucun composant). Plus les jetons et l'audit des classes. Rien n'y est écrit à la main sauf les spécimens de composants. Pas un onglet — une page pour celui qui fabrique l'app. ~1540 lignes. |
...
| **Atelier** (`#atelier`) | La page qui rend tout le vocabulaire côte à côte : les jetons, les **déclinaisons** (l'axe des apparitions), les composants en familles Atomic Design (atomes · molécules · organismes · modales · pages · technique), et l'audit des classes. Tout s'y présente en **vignettes** : fermée, une carte ne montre que la chose, son nom et son nombre de déclinaisons ; ouverte, elle montre toutes les variantes puis **dit ce qu'elle sait de celle qu'on survole**, une ligne par information (description, famille, variante, manette, sélecteur, état, occurrences, où elle est écrite). Le survol ne porte plus de texte, il désigne : une bulle flottante s'en allait au premier mouvement, et deux variantes ne se comparaient jamais. Une **adresse**, pas un onglet : ni compte ni données. Sert à voir ce qu'un changement de thème déplace, et à regarder ce qui existe avant d'inventer un contrôle de plus. `#sink`, son ancienne adresse, y mène toujours. |
| **Apparition** (atelier) | Une combinaison de classes portée par un élément quelque part dans l'app — `button.icon-btn.cal-nav`. L'autre unité du design system, à côté du **composant** : un composant a un nom dans le code et se réutilise ; une apparition n'a que ses étiquettes, et peut n'être écrite qu'à un seul endroit. L'atelier les relit dans les `className=` des sources, sans rien écrire à la main. |
| **Déclinaison** (atelier) | **Une forme à regarder** : une apparition, ou l'un de ses **états** — `.icon-btn` et le même en `.on` ne se ressemblent pas, donc ce sont deux déclinaisons. Rangées par **classe de base**, la première du `className` : `.icon-btn` porte la forme (le rond, 26 px, le liseré), `.cal-nav`, `.sm` ou `.on` la modifient. Ce n'est pas une règle du CSS — aucune classe n'est la sous-classe d'une autre — mais la convention du projet, et c'est ce que dit `.icon-btn.sm` dans la feuille, où la règle n'existe que pour les deux ensemble. Les voir côte à côte est la seule façon de repérer l'intruse. |
