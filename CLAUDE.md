...
| `app.atelier.jsx` | **L'atelier**, page `#atelier`. Il **relit les cinq `.jsx` et `styles.css` à l'exécution** (`fetch`) et en tire **deux inventaires qui ne se recouvrent pas** : les **composants** (une brique du code, trouvée par son annotation `@atelier`) et les **apparitions** (une combinaison de classes qui existe vraiment quelque part — `button.icon-btn.cal-nav`, qui n'est aucun composant). Plus les jetons et l'audit des classes. Rien n'y est écrit à la main sauf les spécimens de composants. Pas un onglet — une page pour celui qui fabrique l'app. ~1600 lignes. |
...
| **Atelier** (`#atelier`) | La page qui rend tout le vocabulaire côte à côte : les jetons, les **déclinaisons** (l'axe des apparitions), les composants en familles Atomic Design (atomes · molécules · organismes · modales · pages · technique), et l'audit des classes. Une **adresse**, pas un onglet : ni compte ni données. Chaque fiche se replie sur son titre et son compte, et porte **dans son corps** — pas dans une bulle au survol — une ligne par nature d'information (apparition, forme, réglages, rôle, occurrences, fichier) : ce que la fiche raconte suit l'élément survolé, mais le bloc, lui, reste lisible. Sert à voir ce qu'un changement de thème déplace, et à regarder ce qui existe avant d'inventer un contrôle de plus. `#sink`, son ancienne adresse, y mène toujours. |
...
| **Déclinaison** (atelier) | Les apparitions qui partagent leur **première classe**, rangées ensemble : `.icon-btn` porte la forme (le rond, 26 px, le liseré), `.cal-nav` ou `.sm` la modifient. Ce n'est pas une règle du CSS — aucune classe n'est la sous-classe d'une autre — mais la convention du projet, et c'est ce que dit `.icon-btn.sm` dans la feuille, où la règle n'existe que pour les deux ensemble. Les voir côte à côte est la seule façon de repérer l'intruse. |
